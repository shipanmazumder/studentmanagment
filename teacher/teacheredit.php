<?php
	...
	if(isset($_FILES['image']))
    {
        ...
        if($update=mysql_query("Update teacher set picture='$file_name' WHERE temail='$temail' ")){
	...
	if(isset($_POST['submit'])){
		...
		$mobile=$_POST['mobile'];
		$email=$_POST['email'];
		if($update=mysql_query("Update teacher SET name='$name',address='$address',birthday='$birthday',gender='$gender', mobile='$mobile',temail='$email'  WHERE teacher_id='$teacher_id' ")){
            header("location:teacher.php");
        }
	}
	if(isset($_POST['password'])){
		...
		$res=mysql_query("SELECT * FROM teacher WHERE temail='$temail'");
		...
		if($oldpassword==$rows['password']){
			...
			{?>
				...
				if($update=mysql_query("Update teacher set password='$newpassword' WHERE temail='$temail' ")){
					header("location:teacher.php");
				}
        	}
		}
  ...
  <body>
  	...
  	<?php require_once("rightsidebar3.php"); ?>
        <div class="left_sidebar">
            ...
            <div class="content3">
            	<span class="t_d"><span class="glyphicon glyphicon-tasks"></span> My Account</span>
                	<div class="a_student">
                    	<form class="form-inline" method="POST" >
                            <table  class="table table-bordered">
                            <thead>
                                <tr>
                                    <th><span class="glyphicon glyphicon-plus-sign"></span> Edit Account</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                ...
                                <tr>
                                	<td align="center"><span style=" margin-left:0px;">Email</span> <input type="email" value="<?php echo $row['temail']; ?>" id="form" name="email" class="form-control" required /></td>
                                </tr>
                                 <tr>
